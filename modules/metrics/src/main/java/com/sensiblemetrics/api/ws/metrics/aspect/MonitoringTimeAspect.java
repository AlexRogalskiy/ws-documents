...
public class MonitoringTimeAspect {
    ...
    @Pointcut("@annotation(com.sensiblemetrics.api.ws.metrics.annotation.MonitoredComponent) " +
            "|| @within(com.sensiblemetrics.api.ws.metrics.annotation.MonitoredComponent)")
    public void monitoredComponentPointcut() {
    }

    @Pointcut("@annotation(com.sensiblemetrics.api.ws.metrics.annotation.MonitoredService) " +
            "|| @within(com.sensiblemetrics.api.ws.metrics.annotation.MonitoredService)")
    public void monitoredServicePointcut() {
    }

    @Pointcut("@annotation(com.sensiblemetrics.api.ws.metrics.annotation.MonitoredRepository) " +
    ...
    @Pointcut("monitoredComponentPointcut()" +
    ...
    public void serviceResponseTimeExceptionAdvice(final JoinPoint joinPoint,
                                                   final Exception ex) {
        this.monitorException(joinPoint, ex, TAG_VALUE_STEREOTYPE_TYPE);
    }

    private Object monitorResponseTime(final ProceedingJoinPoint joinPoint,
        ...
        final Timer timer = this.registry.timer(METER_COMPONENT_TIMER,
                TAG_COMPONENT_CLASS, serviceClass, TAG_METHOD_NAME,
                methodName, TAG_OUTCOME, SUCCESS, TAG_TYPE, type);
        timer.record(stopWatch.getNanoTime(), TimeUnit.NANOSECONDS);

        this.registry.counter(METER_COMPONENT_COUNTER,
                TAG_COMPONENT_CLASS, serviceClass, TAG_METHOD_NAME,
                methodName, TAG_OUTCOME, SUCCESS, TAG_TYPE, type)
                .increment();
        return obj;
    ...
    private void monitorException(final JoinPoint joinPoint,
                                  final Exception ex,
                                  final String type) {
        final String serviceClass = this.getClassName(joinPoint.getThis().getClass().getName());
        final String methodName = joinPoint.getSignature().getName();
        this.registry.counter(METER_COMPONENT_EXCEPTION_COUNTER,
                TAG_EXCEPTION_CLASS, ex.getClass().getName(),
                TAG_COMPONENT_CLASS, serviceClass, TAG_METHOD_NAME,
                methodName, TAG_OUTCOME, ERROR, TAG_TYPE, type)
                .increment();
    }
