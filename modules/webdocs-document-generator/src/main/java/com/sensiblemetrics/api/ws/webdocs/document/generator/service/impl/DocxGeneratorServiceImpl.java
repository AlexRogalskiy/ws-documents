...
import com.sensiblemetrics.api.ws.webdocs.document.generator.handler.DocumentEventHandler;
import com.sensiblemetrics.api.ws.webdocs.document.generator.model.domain.DocumentEvent;
import com.sensiblemetrics.api.ws.webdocs.document.generator.model.domain.MessageData;
import com.sensiblemetrics.api.ws.webdocs.document.generator.model.entity.DocumentEntity;
import com.sensiblemetrics.api.ws.webdocs.document.generator.property.WsDocumentStorageProperty;
import com.sensiblemetrics.api.ws.webdocs.document.generator.property.WsDocumentTemplateProperty;
...
import static com.sensiblemetrics.api.ws.webdocs.commons.utils.ServiceUtils.findInClasspath;
...
public class DocxGeneratorServiceImpl implements DocxGeneratorService {
  ...
  public CompletableFuture<Path> processDocument(final DocumentEntity documentEntity) {
    ...
    final DocumentEventHandler documentEventHandler =
        status -> this.notifyEvent(documentEntity.getId()).accept(status);
   ...
   */
  private Consumer<Path> createDocxFileFromTemplate(
      final DocumentEntity documentEntity, final DocumentEventHandler eventHandler) {
    return path -> {
      try (final InputStream templateInputStream =
          findInClasspath(this.getDocumentTemplateProperty().getNamePattern()).getInputStream()) {
        ...
        log.error(
            "ERROR: cannot generate docx file for document with id: {}, message: {}",
  ...
  private Map<String, String> getMappings(final DocumentEntity documentEntity) {
    ...
    final TypeDescriptor targetType =
        TypeDescriptor.map(
            Map.class, TypeDescriptor.valueOf(String.class), TypeDescriptor.valueOf(String.class));
    return Optional.ofNullable(
            this.conversionService.convert(documentEntity, sourceType, targetType))
    ...
    if (SystemUtils.IS_OS_UNIX) {
      final Set<PosixFilePermission> permissions =
          PosixFilePermissions.fromString(
              this.getDocumentStorageProperty().getPosixFilePermissions());
  ...
  private Consumer<StatusType> notifyEvent(final UUID documentId) {
    return status ->
        this.getEventPublisher()
            .publishEvent(new DocumentEvent(this, MessageData.of(documentId, status)));
  }
}
