...
public class DelegatedCacheLoaderWriter implements CacheLoaderWriter<String, Boolean> {
  ...
  public Map<String, Boolean> loadAll(final Iterable<? extends String> iterable) {
    log.debug(
        String.format(
            "DelegatedCacheLoaderWriter: loading all keys: {%s}", StringUtils.join(iterable, "|")));
    this.getCacheMap().clear();
    Optional.ofNullable(iterable).orElseGet(Collections::emptyList).forEach(this::load);
  ...
  public void write(final String key, final Boolean value) {
    log.debug(
        String.format(
            "DelegatedCacheLoaderWriter: writing entry with key: {%s}, value: {%s}", key, value));
    this.getCacheMap().put(key, value);
  }

  @Override
  public void writeAll(
      final Iterable<? extends Map.Entry<? extends String, ? extends Boolean>> iterable) {
    log.debug(
        String.format(
  ...
  public void deleteAll(final Iterable<? extends String> iterable) {
    log.debug(
        String.format(
            ...
            StringUtils.join(iterable, "|")));
    Optional.ofNullable(iterable).orElseGet(Collections::emptyList).forEach(this::delete);
  }
}
