package com.sensiblemetrics.api.ws.webdocs.commons.enumeration;

import com.sensiblemetrics.api.ws.webdocs.commons.general.annotation.SimpleTest;
import com.sensiblemetrics.api.ws.webdocs.commons.general.annotation.VariableSource;
import com.sensiblemetrics.api.ws.webdocs.commons.general.configuration.TestTag;
...
class ErrorTemplateTypeTest {
  ...
  private static final Stream<Arguments> errorTemplateTypeValues =
      Stream.of(
          Arguments.of(
              ErrorTemplateType.SERVICE_UNAVAILABLE,
              createTypeMatcher("error-0030", "error.service.unavailable")),
          Arguments.of(
              ErrorTemplateType.SERVICE_OPERATION_ERROR,
              createTypeMatcher("error-0040", "error.operation.invalid")),
          Arguments.of(
              ErrorTemplateType.BAD_REQUEST,
              createTypeMatcher("error-0050", "error.request.invalid")),
          Arguments.of(
              ErrorTemplateType.DOCUMENT_PROCESSING_ERROR,
              createTypeMatcher("error-0060", "error.document.processing.invalid")),
          Arguments.of(
              ErrorTemplateType.INVALID_ENDPOINT_CONFIGURATION,
              createTypeMatcher("error-0070", "error.endpoint.configuration.invalid")),
          Arguments.of(
              ErrorTemplateType.INVALID_ENDPOINT_SECURITY_CONFIGURATION,
              createTypeMatcher("error-0080", "error.endpoint.security.configuration.invalid")));

  @ParameterizedTest
  @VariableSource("errorTemplateTypeValues")
  void testCheckErrorTemplateTypeByCodeAndDescription(
      final ErrorTemplateType enumType, final Matcher<ErrorTemplateType> matcher) {
    // then
  ...
  @ParameterizedTest
  @MethodSource
  void testCheckErrorTemplateTypeByName(
      final ErrorTemplateType enumType, final Matcher<String> matcher) {
    // then
  ...
  private static Stream<Arguments> testCheckErrorTemplateTypeByName() {
    return Stream.of(
        ...
        Arguments.of(
            ErrorTemplateType.INVALID_ENDPOINT_SECURITY_CONFIGURATION, equalTo("error-0080")));
  }

  @ParameterizedTest
  @MethodSource
  void testCheckErrorTemplateTypeByValue(
      final String enumName, final Matcher<ErrorTemplateType> matcher) {
    // then
  ...
  private static Stream<Arguments> testCheckErrorTemplateTypeByValue() {
    return Stream.of(
        ...
        Arguments.of(
            "DOCUMENT_PROCESSING_ERROR", equalTo(ErrorTemplateType.DOCUMENT_PROCESSING_ERROR)),
        Arguments.of(
            "INVALID_ENDPOINT_CONFIGURATION",
            equalTo(ErrorTemplateType.INVALID_ENDPOINT_CONFIGURATION)),
        Arguments.of(
            "INVALID_ENDPOINT_SECURITY_CONFIGURATION",
            equalTo(ErrorTemplateType.INVALID_ENDPOINT_SECURITY_CONFIGURATION)));
  }

  @ParameterizedTest
  @MethodSource
  void testCheckErrorTemplateTypeByValueOfWithClass(
      final String enumName, final Matcher<ErrorTemplateType> matcher) {
    // then
  ...
  @ParameterizedTest
  @MethodSource
  void testCheckErrorTemplateTypeWhenPassedCode(
      final String enumCode, final Matcher<ErrorTemplateType> matcher) {
    // then
  ...
  private static Stream<Arguments> testCheckErrorTemplateTypeWhenPassedCode() {
    return Stream.of(
        ...
        Arguments.of(
            "error-0080", equalTo(ErrorTemplateType.INVALID_ENDPOINT_SECURITY_CONFIGURATION)));
  }
  ...
  void testCheckErrorTemplateType_whenPassedNonExistedType() {
    ...
    final IllegalArgumentException thrown =
        assertThrows(
            IllegalArgumentException.class, () -> ErrorTemplateType.valueOf("NON_EXISTED"));

    // then
    assertThat(thrown.getMessage(), startsWith(errorMessage));
  }

  private static CustomTypeSafeMatcher<ErrorTemplateType> createTypeMatcher(
      final String errorCode, final String errorDescription) {
    return new CustomTypeSafeMatcher<ErrorTemplateType>(
        "Compare error code and description message") {
  ...
  private static Function<ErrorTemplateType, CustomTypeSafeMatcher<ErrorTemplateType>>
      createTypeMatcher() {
    return value ->
        new CustomTypeSafeMatcher<ErrorTemplateType>(
            "Compare error template types on a property basis") {
