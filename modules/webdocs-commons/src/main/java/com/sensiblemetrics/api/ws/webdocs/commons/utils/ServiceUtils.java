package com.sensiblemetrics.api.ws.webdocs.commons.utils;
...
public class ServiceUtils {
  ...
  public static final BiConsumer<? super Object, ? super Throwable> DEFAULT_COMPLETABLE_LOG_ACTION =
      (response, error) -> {
        try {
          if (Objects.nonNull(error)) {
            log.info(
                "Canceled completable future request [response={}, error={}]",
                response,
                error.getMessage());
          } else {
  ...
  /**
   * Copies properties from source {@code T} to target {@code T} by input {@link Iterable}
  ...
  public static <T> T copyProperties(
      final T source, final T target, final Collection<String> propsToUpdate) {
    ...
    propsToUpdate.forEach(
        property -> targetWrap.setPropertyValue(property, sourceWrap.getPropertyValue(property)));
    return target;
  }
  ...
  public static <T> Stream<T> streamOf(final T... values) {
    return Arrays.stream(Optional.ofNullable(values).orElseGet(() -> cast(EMPTY_OBJECT_ARRAY)));
  }

  /**
   * Returns non-nullable {@link Stream} of {@code T} by input {@link Iterable} collection of {@code
   * T} values
   *
   * @param <T> type of input element to be converted from by operation
   ...
   */
  @NonNull
  public static <T> Stream<T> streamOf(final Iterable<T> iterable, final boolean parallel) {
  ...
  /**
   * Returns {@link Stream} by transforming input {@link Optional} depending upon whether a value is
   * present
   *
   * @param <T> type of input element to be converted from by operation
   ...
   */
  @NonNull
  public static <T, X extends Throwable> T safeCastOrThrow(
      final Object value, final Class<? extends T> clazz, final X throwable) throws X {
  ...
  public static <T> T safeCast(final Object value, final Class<T> clazz) {
    return safeCastOrThrow(
        value,
        clazz,
        new IllegalArgumentException(
            String.format("Cannot cast input value: {%s}, to class: {%s}", value, clazz)));
  }
