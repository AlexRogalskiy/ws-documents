package com.sensiblemetrics.api.ws.webdocs.commons.helper;
...
import static com.sensiblemetrics.api.ws.webdocs.commons.utils.ServiceUtils.doThrow;
...
public class OptionalConsumer<T> {
  ...
  @NonNull private final Optional<T> optional;
  ...
  /**
   * Applies a mapping operation to the argument only when {@code test} returns {@code true} for
   * that argument.
   *
   ...
   * @return OperatorType appling {@code ifTrue} when {@code test} is true and otherwise returning
   *     the argument.
   */
  public static <T> UnaryOperator<T> applyIf(
      final Predicate<T> test, final UnaryOperator<T> ifTrue) {
  ...
  /**
   * Convert a consumer to a function that consumes the argument and returns {@link
   * Optional#empty()}.
   *
   ...
   */
  @NonNull
  public <X extends Exception> OptionalConsumer<T> ifPresent(
      @NonNull final ThrowingConsumer<T, X> consumer) {
    Assert.notNull(consumer, "Consumer should not be null");
   ...
   */
  @NonNull
  public <X extends Exception> OptionalConsumer<T> ifNotPresent(
      @NonNull final ThrowingRunnable<X> executor) {
  ...
  /**
   * Returns a {@link Stream} wrapper for the {@code T} value. The resulting stream contains either
   * the value is present or it is empty if the value is empty.
   *
  ...
  /**
   * Return the contained {@code T} value, if present, otherwise throw an {@code X} exception to be
   * created by the provided {@link Supplier}
   *
   * @param <X> type of the exception to be thrown
   * @param exceptionSupplier - initial input {@link Supplier} which will return the exception to be
   *     thrown, must not be {@literal null}
   * @return the present {@code T} value
   * @throws X if there is no {@code T} value present
   */
  @NonNull
  public <X extends Throwable> T getValueOrElseThrow(final Supplier<? extends X> exceptionSupplier)
      throws X {
   ...
   *
   * @param other - initial input {@code T} value to be returned if there is no value present, may
   *     be null
  ...
  /**
   * Return the {@code T} value if present, otherwise invoke {@code T} other and return the result
   * of that invocation.
   *
   * @param otherSupplier - initial input {@link Supplier} whose result is returned if no value is
   *     present, must not be {@literal null}.
  ...
  /**
   * Returns binary result whether current {@link OptionalConsumer} is present and not equal to
   * input {@link OptionalConsumer}
   *
   * @param <E> type of configurable entity
   * @param other - initial input {@link OptionalConsumer} to compare by
   * @return true - if current {@link OptionalConsumer} is present and not equal to input {@link
   *     OptionalConsumer}, false - otherwise
   */
  ...
  /**
   * Returns binary result whether current {@link OptionalConsumer} is present and equal to input
   * {@link OptionalConsumer}
   *
   * @param <E> type of configurable entity
   * @param other - initial input {@link OptionalConsumer} to compare by
   * @return true - if current {@link OptionalConsumer} is present and equal to input {@link
   *     OptionalConsumer}, false - otherwise
   */
  ...
  public <X extends Exception>
      ThrowingFunction<Consumer<T>, ThrowingFunction<Runnable, OptionalConsumer<T>, X>, X>
          ifPresent() {
  ...
  @NonNull
  public <X extends Exception>
      ThrowingFunction<Runnable, ThrowingFunction<Consumer<T>, OptionalConsumer<T>, X>, X>
          ifNotPresent() {
   ...
   */
  @Nullable
  public <R> OptionalConsumer<R> flatMap(
      final Function<? super T, ? extends OptionalConsumer<R>> mapper) {
   ...
   */
  private static <X, Y, R, E extends Exception>
      ThrowingFunction<X, ThrowingFunction<Y, R, E>, E> curry(final BiFunction<X, Y, R> function) {
   ...
   */
  private static <X, Y, R, E extends Exception> BiFunction<X, Y, R> uncurry(
      final ThrowingFunction<X, ThrowingFunction<Y, R, E>, E> function) {
  ...
  /**
   * Returns current {@link OptionalConsumer} by operating on input {@link Consumer} and {@link
   * Runnable} operators
   *
   ...
   */
  private OptionalConsumer<T> ifPresent(final Consumer<T> present, final Runnable notPresent) {
    if (this.optional.isPresent()) {
  ...
  /**
   * Returns current {@link OptionalConsumer} by operating on input {@link Runnable} and {@link
   * Consumer} operators
   *
   ...
   * @return current {@link OptionalConsumer}
   */
  private OptionalConsumer<T> ifNotPresent(final Runnable notPresent, final Consumer<T> present) {
    if (!this.optional.isPresent()) {
      notPresent.run();
    } else {
      present.accept(this.optional.get());
  ...
  /**
   * Invokes the given {@link Consumer} if the {@link Optional} is present or the {@link Executable}
   * if not
   *
   * @param optional - initial input {@link Optional} value
  ...
  public static <T> void ifPresentOrElse(
      final Optional<T> optional, final Consumer<T> consumer, final Runnable runnable) {
