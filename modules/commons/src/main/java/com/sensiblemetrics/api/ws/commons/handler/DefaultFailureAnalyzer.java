...
public class DefaultFailureAnalyzer
    extends AbstractFailureAnalyzer<BeanNotOfRequiredTypeException> {

  /**
   * Returns an analysis of the given {@code failure}, or {@code null} if no analysis was possible.
   *
  ...
  protected FailureAnalysis analyze(
      final Throwable exception, final BeanNotOfRequiredTypeException cause) {
  ...
  private String getDescription(final BeanNotOfRequiredTypeException cause) {
    return String.format(
        ...
        cause.getBeanName(), cause.getRequiredType().getName(), cause.getActualType().getName());
  }
