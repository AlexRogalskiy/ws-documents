...
import com.sensiblemetrics.api.ws.commons.exception.ServiceOperationException;
...
public class DelegatedTimeoutCallableProcessingInterceptor extends TimeoutCallableProcessingInterceptor {
     ...
     */
    @Override
    public <T> Object handleTimeout(final NativeWebRequest request,
                                    final Callable<T> task) {
        return Optional.ofNullable(request.getNativeRequest(HttpServletRequest.class))
                .map(req -> {
                    log.error("Timeout has been reached before sending response to client, request: {}", req.getRequestURI());
